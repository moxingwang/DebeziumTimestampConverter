...
public abstract class DebeziumTimestampConverter<R extends ConnectRecord<R>> implements Transformation<R> {
    ...
    static {
        TRANSLATORS.put(TYPE_STRING, new TimestampTranslator() {
            ...
            public Date toRaw(Config config, Object orig) {
                ...
                try {
                    String dateValue = (String) orig;

                    //todo 正则判断
                    if (dateValue.contains("'T'")) {
                        return config.utcFormatToGMT.parse((String) orig);
                    } else {
                        return config.format.parse(dateValue);
                    }
    ...
    public static class Config {
        ...
        SimpleDateFormat utcFormatToGMT;
        String[] field_type;
        String[] struct_field;

        void init() {
            utcFormatToGMT = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'");
            utcFormatToGMT.setTimeZone(TimeZone.getTimeZone("GMT"));
        }
    }
